...
function ViewAllFormsAdmin() {
  const [forms, setForms] = useState([]);
  const [formsToUse, setFormsToUse] = useState([]);
  ...
  useEffect(() => {
    const init = async () => {
      const formData = await Firebase.getAllForms();
      setForms(formData);
      setFormsToUse(formData);
    };
  ...
  useEffect(() => {
    ...
    if (filter === FILTERS.NEEDS_REVIEW) {
      setFormsToUse(forms.filter(form => form.status === Status.NEEDS_REVIEW));
    }
    ...
    <Box bg="#F2F5F9" minH="100vh" py="2rem">
      ...
      >
        <Flex alignItems="center">
          <Heading size="lg">View Forms Admin</Heading>
          <Text ml="1rem">Filters: </Text>
          <RadioGroup ml="1rem" onChange={setFilter} value={filter}>
            <Stack direction="row">
              <Radio value={FILTERS.NONE}>None</Radio>
              <Radio value={FILTERS.NEEDS_REVIEW}>Needs Review</Radio>
            </Stack>
          </RadioGroup>
        </Flex>
        <Button
...
export default ViewAllFormsAdmin;
